...
class VendorTaxReportWizard(models.TransientModel):
    _name = "vendor.tax.report.wizard"
    _description = "Vendor Tax Report Excel Export Wizard"
    ...
    def action_export_excel(self):
        """Generate and download Excel report for Vendor Bills"""
        self.ensure_one()

        # Create Excel file in memory
        output = io.BytesIO()
        workbook = xlsxwriter.Workbook(output, {"in_memory": True})
        worksheet = workbook.add_worksheet("Vendor Bills Report")
        ...
        # Get vendor bills data - Only Vendor Bills and Refunds
        domain = [
        ...
        bills = self.env["account.move"].sudo().search(domain, order="invoice_date, name")
        ...
        for bill in bills:
            ...
            partner_vat = bill.partner_id.vat if bill.partner_id else ""

            # Write invoice date
            if bill.invoice_date:
                worksheet.write_datetime(row, 0, bill.invoice_date, date_format)
            else:
                worksheet.write(row, 0, "", cell_format)

            worksheet.write(row, 1, bill.name or "", cell_format)
            worksheet.write(row, 2, bill.ref or "", cell_format)
            worksheet.write(row, 3, partner_name, cell_format)
            worksheet.write(row, 4, partner_vat or "", cell_format)
            worksheet.write(row, 5, bill.amount_untaxed, number_format)
            worksheet.write(row, 6, bill.amount_tax, number_format)
            worksheet.write(row, 7, bill.amount_total, number_format)
            row += 1
        ...
        filename = "Vendor_Bills_Report_%s_to_%s.xlsx" % (
        ...
        return {
            ...
            "res_model": "vendor.tax.report.wizard",
            "view_mode": "form",
            "res_id": self.id,
            "views": [(False, "form")],
            "target": "new",
